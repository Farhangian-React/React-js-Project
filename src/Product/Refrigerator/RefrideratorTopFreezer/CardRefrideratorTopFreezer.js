   ...
   return (
    ...
    <Box sx={{mt:2,mb:4}}>
      <ul className='headers'>
        <li>محصولات / </li>
        <li> یخچال و فریزر / </li>
        <li> یخچال های فریزر بالا </li>
      </ul>
    </Box>
    <Box>
      <Typography sx={{fontSize:{xs:'14px',lg:'18px'}, px:5,pb:2}}>در   sohil بهترین انتخاب از یخچال های بالا را بیابید. یخچال فریزرها معمولاً مقرون به صرفه هستند، برای فضاهای کوچکتر راحت‌تر هستند و فریزر را در سطح چشم شما قرار می‌دهند.
      </Typography>
      <Typography  sx={{fontSize:{xs:'14px',lg:'18px'},px:5,pb:2}}>ما استرس ناشی از یافتن یخچال عالی برای شما را از بین می بریم. ما مفتخریم که یخچال‌ها را با هر قیمتی ارائه می‌کنیم تا بتوانید آنچه را که نیاز دارید را بدون شکستن بانک پیدا کنید. با انتخاب محدوده بودجه خود شروع کنید، سپس ظرفیت متناسب با نیازهای خانوار خود را با ابعاد متناسب با فضای اختصاص داده شده در آشپزخانه خود انتخاب کنید. سپس سبک مورد علاقه خود را برای تکمیل و بیشتر انتخاب کنید! هنگامی که یکی را که دوست دارید پیدا کردید، برای خرید آن را به سبد خرید خود اضافه کنید و ضمانت های خود را اضافه کنید یا با مدل های دیگر مقایسه کنید تا انتخاب کاملی داشته باشید.
      </Typography>
    </Box>
...
export default function CardRefrideratorTopFreezer({ itemsPerPage }) {
 ...
 const getData=()=>{
  fetch('http://localhost:3000/RefrideratorTopFreezer')
  .then(res => res.json())
  .then((result)=> {
    setData(result);
setDatashow(result);
